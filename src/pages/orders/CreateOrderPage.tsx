...
export default function CreateOrderPage() {
  ...
  const { customers, vendors, inventory, addOrder, orders } = useBusiness();
  const [loading, setLoading] = useState(false);

  // Auto-generate order number
  const defaultOrderNo = `ORD-${new Date().getFullYear()}-${String(orders.length + 1).padStart(3, '0')}`;
  ...
  const onSubmit = async (data: FormData) => {
    setLoading(true);
    try {
      const party = data.type === 'sale' 
        ? customers.find(c => c.id === data.partyId)
        : vendors.find(v => v.id === data.partyId);
      ...
      toast.success('Order created securely');
      navigate('/orders');
    } catch (e: any) {
      console.error(e);
    ...
    <div style={{ maxWidth: 900 }}>
      <div className="page-header" style={{ marginBottom: 16 }}>
        ...
        <div className="page-header-left" style={{ flex: 1 }}>
          <h2>Create Order</h2>
          <p>Draft a sales or purchase order without affecting stock until fulfilled.</p>
        </div>
      </div>
        ...
        <div className="card card-pad" style={{ marginBottom: 24 }}>
          <div className="form-grid form-grid-3">
            <div className="form-group">
              <label className="form-label required">Order Type</label>
              <select 
                {...register('type')} 
              ...
              >
                <option value="sale">Sales Order (Customer)</option>
                <option value="purchase">Purchase Order (Vendor)</option>
              </select>
            </div>

            <div className="form-group">
              <label className="form-label required">{watchType === 'sale' ? 'Customer' : 'Vendor'}</label>
              <select {...register('partyId')} className={`form-control ${errors.partyId ? 'error' : ''}`}>
              ...
              {errors.partyId?.message && <span className="form-error">{String(errors.partyId.message)}</span>}
            </div>

            <div className="form-group">
              <label className="form-label required">Order Number</label>
              <input {...register('orderNumber')} className="form-control" />
            </div>
                          ...
                          onChange={(e) => {
                            ...
                            if (val) {
                              ...
                              if (item) {
                                ...
                                setValue(`lineItems.${index}.rate`, watchType === 'sale' ? item.sellingPrice : item.costPrice);
                              }
                            } else {
        ...
        <div style={{ display: 'flex', gap: 24, alignItems: 'flex-start' }}>
          ...
          <div className="card card-pad" style={{ width: 340 }}>
            ...
            <button type="submit" className="btn btn-primary" style={{ width: '100%' }} disabled={loading}>
              {loading ? <span className="spinner spinner-sm" /> : 'Save Order'}
            </button>
          </div>
        </div>
      </form>
